...
| **部署網址** | [點擊前往 App](https://aiothw5-3itlsghnk3qtmx4ub9ttrj.streamlit.app/) |

---

## 1. 📝 專案簡介 (Abstract)
本專案參考課程中關於 **檢索增強生成（RAG）** 的原理，開發了一個基於 Streamlit 的互動式網頁應用程式。系統整合了 **LangChain 框架** 與 **Google Gemini 模型**，旨在解決大型語言模型在面對私有或特定領域文件時的幻覺問題。
...
> **⚠️ 注意：** 使用時請在左側欄位輸入有效的 **Google Gemini API Key** (可於 Google AI Studio 免費申請)。

## 3. 🛠️ 使用技術棧 (Tech Stack)
* **前端介面：** `Streamlit`
* **LLM 框架：** `LangChain` (v0.2 穩定版)
* **語言模型：** Google `gemini-pro`
* **Embedding 模型：** Google `models/text-embedding-004`
...
## 5. ✨ 功能特點
* **支援多語言 PDF**：針對中文語境優化了 Text Splitter 的切分符號，支援中英文文件。
